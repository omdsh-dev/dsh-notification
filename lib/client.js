...
function pendingNotificationTag(sessionId) {
...
var zh = {
  ...
  "settings.pending.subtitle": "\u5F53 DSH \u7B49\u5F85\u4F60\u5BA1\u6279\u6216\u56DE\u7B54\u63D0\u95EE\u65F6\u63D0\u9192\uFF0C\u5207\u5230\u5176\u4ED6\u6807\u7B7E\u9875\u4E5F\u4E0D\u4F1A\u9519\u8FC7\u3002",
  "settings.pending.approval": "\u7B49\u5F85\u5BA1\u6279",
  "settings.pending.question": "\u7B49\u5F85\u56DE\u7B54\u63D0\u95EE",
  ...
  "notify.titleQuestion": "DSH \u9700\u8981\u4F60\u56DE\u7B54\u95EE\u9898",
  "notify.titlePlanReview": "DSH \u9700\u8981\u4F60\u8BC4\u5BA1\u8BA1\u5212",
  "notify.pendingBodyFallback": "\u6709\u5F85\u5904\u7406\u7684\u64CD\u4F5C",
...
var en = {
  ...
  "settings.pending.subtitle": "Get notified when DSH waits for your approval or an answer, even while on another tab.",
  "settings.pending.approval": "Awaiting approval",
  "settings.pending.question": "Awaiting your answer",
  ...
  "notify.pendingBodyFallback": "There is a pending action",
...
function pendingAdvance(prev, kind) {
  ...
  const fresh = kind !== void 0 && kind !== prev.kind;
  return { kind, fresh };
}
function pendingNotificationFor(sessionId, origin, title, kind, settings) {
  if (origin === "subagent") return null;
...
function apply(ctx) {
  ...
  ctx.effect(() => {
    const observedPending = /* @__PURE__ */ new Map();
    const reseed = () => {
    ...
    const off = sessions.list.subscribe(() => {
      ...
      for (const id of state.ids) {
        ...
        if (!fresh || nextKind === void 0) continue;
        ...
        const showIt = shouldShow(permission, current.backgroundOnly, document.hidden, id, state.current);
        console.info(
          `[dsh-notification] pending ${nextKind} ${id}: show=${showIt} (permission=${permission} backgroundOnly=${current.backgroundOnly} hidden=${document.hidden} current=${String(state.current)})`
        );
        if (showIt) {
          show(
